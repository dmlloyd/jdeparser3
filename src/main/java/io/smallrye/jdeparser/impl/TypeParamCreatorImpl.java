...
public final class TypeParamCreatorImpl extends AbstractCreator implements TypeParamCreator, Writable {
    ...
    /** {@inheritDoc} */
    @Override
    public void extends_(List<Type> bounds) {
        checkActive();
        ...
        Type ourBounds = this.bounds;
        if (ourBounds == null) {
            this.bounds = switch (bounds.size()) {
                case 0 -> null;
                case 1 -> bounds.get(0);
                default -> new IntersectionType(bounds);
            };
        } else if (ourBounds instanceof IntersectionType ourIntersection) {
            this.bounds = switch (bounds.size()) {
    ...
    public void write(final SourceFileWriter writer) throws IOException {
        ...
        if (bounds != null) {
            ...
            AbstractExpr.writeType(writer, bounds);
        }
    }
}
